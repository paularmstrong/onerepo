...
<span slot="content">In the `vitest.workspace.ts` file, we need to duplicate the workspace globs from our original `package.json` or `pnpm-workspace.yaml` file and ensure they point to valid Vitest config files:</span>
<span slot="terminal">
