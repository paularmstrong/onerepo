...
```ansi title="Type prefixes"
 [34m[1mINFO[0m General information that should always be shared
 [31m[1mERR[0m  Error output. The command will exit with a non-zero exit code.
 [33m[1mWRN[0m  Worth calling out, but things will continue without a problem
 [36m[1mLOG[0m  Slightly more verbose information when it's helpful
 [35m[1mDBG[0m  Included when you really want to know what's goin on
```
...
```ansi title="Log types within a step"
 ...
 │ [34m[1mINFO[0m General information that should always be shared
 │ [31m[1mERR[0m  Some error output information
 │ [31m[1mERR[0m  that may be on multiple lines
 │ [36m[1mLOG[0m  Slightly more verbose information when it's helpful
 └ [31m✘[39m [2m0ms[0m
```
...
<div class="grid grid-cols-2">
	<div>
		```ansi title="Completed successfully" [1;2m■ [0;32m✔ [0mCompleted [2m5ms[0m ``` This command finished cleanly with
		exit code `0`.
	</div>
	<div>
		```ansi title="Completed, but failed" [1;2m■ [0;31m✘ [0mCompleted with errors [2m5ms[0m ``` This command did _not_
		exit cleanly and provide a non-zero exit code.
	</div>
</div>
