...
| ------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`        | `string \| Array<string>`                | The command’s name. If provided as an array, all values will be considered aliases for the same command.<br/><br/>Any value can also be `'$0'`, which is a special token that allows this command to be the default for the [parent command](#nested-commands). |
...
export const description =
	'Create a new branch using our team’s standard naming format.';
...
export const handler: Handler<Args> = async (argv, { logger }) => {
	...
	logger.info(
		`TODO: create new branch for #${issue} with description "${description}"`,
	);
};
```
...
export const description =
	'A basic command lists workspaces based on input arguments';
...
export const handler: Handler<Argv> = async (
	argv,
	{ getWorkspaces, logger },
) => {
...
Finally, running out command, we can see list of workspaces based on those added input argument flags:
...
export const handler: Handler = async (argv, { logger }) => {
	// -v (1)
	logger.error(
		'This will output an error (and also set cause this run to exit with code 1)',
	);
