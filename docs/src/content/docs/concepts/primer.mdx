...
Repositories are collections of files and folders managed by a [vcs](#version-control-system) that track changes to software.

### Monorepo

A [repository](#repository) that includes more than one project, typically with inter-connected and shared dependencies between the projects.

### Root

The root folder of a [repository](#repository). This term may also be used to refer to the root [workspace](#workspace) of a [monorepo](#monorepo).

### Workspace

A workspace is a singular project within a [monorepo](#monorepo) that is either able to be published, deployed, or imported as a dependency of another workspace.

### Graph

The graph is a representation of the [workspaces](#workspace) interdependencies within a [monorepo](#monorepo).
